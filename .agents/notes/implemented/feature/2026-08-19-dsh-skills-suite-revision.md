...
The two published trees previously relied on a prose warning in the 0.2.0 notes to stay in sync. `scripts/verify-dsh-skills-sync.ts` now asserts byte equality between the two published skills trees in both directions, plus version agreement across `VERSION`, every SKILL.md frontmatter, the Codex manifest, and the provider's hardcoded METADATA. It runs in the `doc-sync` gate list.
